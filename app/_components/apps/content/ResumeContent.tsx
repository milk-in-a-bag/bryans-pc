      ...
      <div className="max-w-3xl flex flex-col gap-7">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#0078d4] hover:bg-[#1084d8] transition-colors text-sm font-medium text-white"
          >
            <svg
              width="13"
              height="13"
        ...
        <ResumeSection title="Experience">
          {EXPERIENCE.map((item) => (
            <TimelineEntry key={item.title + item.org} item={item} />
          ))}
        </ResumeSection>
        ...
        <ResumeSection title="Skills">
          <div className="grid grid-cols-2 gap-4">
            {SKILLS_GROUPS.map((group) => (
              <div key={group.label}>
                <p className="text-[11px] font-semibold text-white/35 mb-2">
                  {group.label}
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {group.items.map((item) => (
                    <span
                      key={item}
                      className="text-[11px] px-2 py-0.5 rounded bg-white/10 text-white/65 border border-white/[0.08]"
                    >
                      {item}
                    </span>
                  ))}
